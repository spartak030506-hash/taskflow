...
class TestTaskUpdateAPI:
    ...
    def test_update_task_admin_success(self, api_client, project_for_tasks, task, project_admin_user):
    ...
    def test_update_task_other_member_forbidden(self, api_client, project_for_tasks, task):
        ...
        ProjectMemberFactory(project=project_for_tasks, user=other_member, role=ProjectMember.Role.MEMBER)
...
class TestTaskDeleteAPI:
    ...
    def test_delete_task_admin_success(self, api_client, project_for_tasks, task, project_admin_user):
        api_client.force_authenticate(user=project_admin_user)
        url = reverse('task-detail', kwargs={'project_pk': project_for_tasks.pk, 'pk': task.pk})
    ...
    def test_delete_task_assignee_forbidden(self, api_client, project_for_tasks, task_with_assignee):
        task, assignee = task_with_assignee
        api_client.force_authenticate(user=assignee)
        url = reverse('task-detail', kwargs={'project_pk': project_for_tasks.pk, 'pk': task.pk})
